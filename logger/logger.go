...
func (d *paxosLogger) createILogger() {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.logger == nil {
...
func (d *paxosLogger) SetLevel(l LogLevel) {
	d.createILogger()
	d.logger.SetLevel(l)
}

func (d *paxosLogger) Debugf(format string, args ...interface{}) {
	d.createILogger()
	d.logger.Debugf(format, args...)
}

func (d *paxosLogger) Infof(format string, args ...interface{}) {
	d.createILogger()
	d.logger.Infof(format, args...)
}

func (d *paxosLogger) Warningf(format string, args ...interface{}) {
	d.createILogger()
	d.logger.Warningf(format, args...)
}

func (d *paxosLogger) Errorf(format string, args ...interface{}) {
	d.createILogger()
	d.logger.Errorf(format, args...)
}

func (d *paxosLogger) Panicf(format string, args ...interface{}) {
	d.createILogger()
...
type sysLoggers struct {
	mu            sync.Mutex
	loggers       map[string]*paxosLogger
...
func createSysLoggers() *sysLoggers {
	s := &sysLoggers{
		loggers: make(map[string]*paxosLogger),
	}
	return s
}
...
func createDefaultILogger(pkgName string) ILogger {
	...
	return CreateLogrusLog(pkgName)
}
